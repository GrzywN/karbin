...
export async function getStaticProps({ locale }) {
  const quickIntroductionSectionContent = getParsedJsonContent(
    locale,
    join(CONTENT_PATH, 'quick-introduction-section')
  ) as QuickIntroductionSectionProps;

  const myWorkSectionContent = getParsedJsonContent(
    locale,
    join(CONTENT_PATH, 'my-work-section')
  ) as MyWorkSectionProps;

  const blogPostsSectionContent = getParsedJsonContent(
    locale,
    join(CONTENT_PATH, 'blog-posts-section')
  ) as BlogPostsSectionProps;

  const contactSectionContent = getParsedJsonContent(
    locale,
